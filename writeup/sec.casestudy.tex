...
\textbf{Analysis and Visualization Examples}
\begin{itemize}
\item Entry topic mixture: visualize D-T.$\theta$ as pie chart for each entry P(Topic|Entry)
\item Topic TopNEntries: entry-token-topic groupby topic and entry; groupcount the tokens; sort by count for each topic $\rightarrow$ top entries per topic
\item Topic TopNWords: token-word-topic groupby word and topic; groupcount; sort by count $\rightarrow$ Topics visualized as top words / visualize T-W.$\mu$ for each topic P(Word|Topic)
\item Topic TopNFrames: filter frames where noun and verb belong to the same topic $\rightarrow$ topic frames; topicframes-token-word-topic groupby topic and startframe + endframe word; groupcount the frame occurrences; sort by count $\rightarrow$ Topics visualized as top frames
\item Topic Time: entry groupby timestamp monthly $\rightarrow$ Entry.Month, Entry.Year; token-entry-topic groupby topic and Entry.Month + Entry.Year; Visualize groupcount of tokens assigned to topics for every month as timeline $\rightarrow$ see importance of topic over time
\end{itemize}
