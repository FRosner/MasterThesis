...
Beside DGMs, with or without plate notation, there are other ways to express probabilistic models. The most exact way is to use \emph{mathematical formulas}, maybe combined with an algorithmic description. Other graphical modeling languages are \emph{undirected graphical models}, \emph{factor graphs}, that all focus on random variables and independence assumptions, as well as \emph{probabilistic relational models} and \emph{probabilistic entity-relationship models}, that try to combine relational data modeling and probabilistic modeling.
